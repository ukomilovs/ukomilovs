...
select * from Employees
select EmployeeID, coalesce(FirstName, LastName) as PreferredName
from Employees 
...
select * from Employees
order by Salary desc offset 11 rows fetch next 9 rows only
...
select EmployeeID, ISNULL(FirstName, ' ') + ' ' + ISNULL(LastName, ' ') as FullName
from Employees
