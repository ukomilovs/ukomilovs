...
select COUNT(distinct category) as uniqueproducts from Products

select ProductID, SUM(SaleAmount) as sum
from Sales
group by ProductID
having ProductID = 7
...
select DepartmentName, COUNT(distinct EmployeeID) from Employees
...
select departmentname, avg(Salary) avg_salary from Employees
group by DepartmentName
...
select YEAR(Orderdate) as peryear, max(quantity) maxOrderQuantity, min(quantity) minOrderQuantity from Orders
