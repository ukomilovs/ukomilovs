...
--Easy-Level Tasks
select Category, sum(StockQuantity) total_products from Products
group by Category

select avg(Price) [avg price electronics] from Products
group by Category
...
select Country, count(CustomerID) [Total Customers] from Customers
group by Country

select min(Quantity) [min quantity ordered], max(Quantity) [max quantity ordered] from Orders
...
from Products
--
select district_id, district_name, [2012], [2013] from city_population
pivot
(
...
select district_id, year, [Bektemir], [Chilonzor], [Yakkasaroy] from city_population
pivot
(
...
select district_id, district_name, newyear, newpopulation from Population_Each_Year
unpivot
(
	newpopulation for newyear in ([2012], [2013])
) as pivot_table1

select Products.ProductName, COUNT(Sales.ProductID) as [num of times each has been sold]
from Products
inner join Sales on Products.ProductID = Sales.ProductID
group by Products.ProductName

select district_id,[new district_name], [newpopulation] year from Population_Each_City
unpivot
(
	[newpopulation] for [new district_name] in ( [Bektemir], [Chilonzor], [Yakkasaroy])
) as pivot_table2
